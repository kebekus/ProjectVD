...
private lemma max_cartanKernel_le (f : ℂ → ℂ) (R α β : ℝ) :
...
private lemma measurable_cartanKernel {f : ℂ → ℂ} (hf : Measurable f) {R : ℝ} :
...
private lemma integral_norm_cartanKernel_eq (f : ℂ → ℂ) (R β : ℝ) :
  ...
  have h_kernel :
    ...
    rw [integral_restrict_Ioc_eq_intervalIntegral two_pi_pos.le]
...
private lemma integrable_integral_norm_cartanKernel {f : ℂ → ℂ} (h : Meromorphic f) {R : ℝ} :
    Integrable
...
private lemma cartan_integrability {f : ℂ → ℂ} (h : Meromorphic f) {R : ℝ} :
