...
theorem pos_radius_of_mem_ball {c : ℂ} (hw : w ∈ ball c R) :
    0 < R := by
  by_contra hCon
  simp_all [ball_eq_empty.2 (not_lt.mp hCon)]
...
where `U` is a disk, one consider a similar decomposition, called "Canonical
Decomposition" or "Blaschke Product" that replaces the factors `(· - u)` by
"canonical factors" that take only values of norm one on the boundary of the
circle. This file introduces the canonical factors.

See Page 160f of [Lang, *Introduction to Complex Hyperbolic Spaces*][MR886677]
...
/--
Given `R : ℝ` and `w : ℂ`, the canonical factor is the function `fun z ↦ (R ^ 2
- (conj w) * z) / (R * (z - w))`. In applications, on will typically consider a
setting where `w ∈ `.
-/
...
theorem meromorphicOrderAt_canonicalFactor (h : w ∈ ball 0 R) :
  ...
  have : meromorphicOrderAt (fun z ↦ ↑R ^ 2 - (starRingEnd ℂ) w * z) w = 0 := by
    ...
    · rw [← normSq_eq_conj_mul_self, normSq_eq_norm_sq w, sub_ne_zero, ne_eq, ← ofReal_pow, ofReal_inj,
        sq_eq_sq₀ (pos_radius_of_mem_ball h).le (norm_nonneg w)]
      rw [mem_ball_iff_norm, sub_zero] at h
      grind
  simp [this, meromorphicOrderAt_const, (pos_radius_of_mem_ball h).ne', meromorphicOrderAt_id_sub_const]
  ...
  have h_num_ne_zero : R^2 - (conj w) * z ≠ 0 := by
    ...
    have : ‖(starRingEnd ℂ) w * z‖ < R ^ 2 := by
      simpa [Complex.norm_mul, RCLike.norm_conj, pow_two] using
        mul_lt_mul' (mem_ball_zero_iff.mp hw).le h₁z dist_nonneg (pos_radius_of_mem_ball hw)
    simp [← h] at this
  exact div_ne_zero h_num_ne_zero h_denom_ne_zero

theorem nonzero_canonicalFactor' {z : ℂ} (hw : w ∈ ball 0 R) (h₁z : z ∈ closedBall 0 R) (h₂z : z ≠ w)  :
    CanonicalFactor R w z ≠ 0 := by
  have h_denom_ne_zero : R * (z - w) ≠ 0 := by
    apply mul_ne_zero _ (sub_ne_zero.2 h₂z)
    rw [ne_eq, ofReal_eq_zero]
  ...
  have h_num_ne_zero : R^2 - (conj w) * z ≠ 0 := by
    ...
    have : ‖(starRingEnd ℂ) w * z‖ < R ^ 2 := by
      simpa [pow_two] using
        mul_lt_mul (mem_ball_zero_iff.mp hw) (mem_closedBall_zero_iff.mp h₁z) (norm_pos_iff.mpr hz) (pos_of_mem_ball hw).le
    simp [← h] at this
  exact div_ne_zero h_num_ne_zero h_denom_ne_zero
...
theorem norm_canonicalFactor_eval_circle_eq_one {z : ℂ} (hw : w ∈ ball 0 R) (hz : z ∈ sphere 0 R) :
  ...
  have : (starRingEnd ℂ) z * z = R ^ 2 := by
    ...
    exact conj_mul' z
  rw [← this]
  rw [← mul_sub_right_distrib, norm_mul, norm_mul, hz]
  rw [← (starRingEnd ℂ).map_sub, RCLike.norm_conj, mul_comm]
  congr
  simp [abs_of_pos (pos_radius_of_mem_ball hw)]
  simp only [Complex.norm_mul, norm_real, norm_eq_abs, ne_eq, mul_eq_zero, abs_eq_zero,
    norm_eq_zero, not_or]
  constructor
  · linarith [pos_radius_of_mem_ball hw]
  · by_contra hCon
