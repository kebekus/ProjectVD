...
If `f : ℂ → ℂ` is meromorphic, this file establishes a presentation of the
proximity function `proximity f ⊤` as iterated circle averages. This statement
can be used to compare the proximity- and logarithmic counting functions, and is
one of the key ingredients in the proof of Cartan's classic formula for the
...
The proof of the integral presentation of the proximity function relies on an
extended computation, applying Fubini's theorem to the Cartan kernel of
integration. This section defines the kernel and establishes its integrability,
as a function of two variables.
-/

/--
Given `f : ℂ → ℂ` and `R : ℝ`, define the Cartan kernel of integration as the
function `α β ↦ log ‖f (circleMap 0 R β) - circleMap 0 1 α‖`.
-/
...
/--
If `f : ℂ → ℂ` is measurable, then the Cartan kernel of integration is
measurable, as a function in the two variables `α` and `β`.
-/
...
/--
If `f : ℂ → ℂ` is meromorphic, then the Cartan kernel of integration is
...
/--
Complementary statement to `proximity_top_eq_circleAverage_circleAverage`,
providing circle integrability of the integrand.
-/
theorem circleIntegrable_circleAverage_log_norm_sub
  ...
  have integrable_intervalIntegral_cartanKernel_left :
    ...
    rw [uIoc_of_le two_pi_pos.le, IntegrableOn, Measure.volume_eq_prod,
      ← Measure.prod_restrict] at h_int
    simpa [intervalIntegral.integral_of_le two_pi_pos.le] using h_int.integral_prod_left
