...
As a corollary, Cartan's formula implies the (surprising non-trival) fact that
the characteristic function is monotone.

At present, this file establishes circle integrability of the function `a ↦ log
‖meromorphicTrailingCoeffAt (f · - a) 0‖` and computes values of the circle
integral.
...
private lemma log_trailingCoeff_eq_zero_on_unitSphere {a : ℂ}
    (h : 0 < meromorphicOrderAt f 0) (ha : a ∈ sphere 0 |1|) :
...
private lemma eventuallyEq_log_trailingCoeff_of_meromorphicOrderAt_eq_zero
    (h : MeromorphicAt f 0) (hzero : meromorphicOrderAt f 0 = 0) :
...
a) 0‖` that appears in Cartan's formula.
-/
...
/--
Circle average of the function `fun a ↦ log ‖meromorphicTrailingCoeffAt (f · -
a) 0‖` that appears in Cartan's formula, in case where `f` has a zero at the
origin.
-/
...
/--
Circle average of the function `fun a ↦ log ‖meromorphicTrailingCoeffAt (f · -
a) 0‖` that appears in Cartan's formula, in case where  `f` has order zero at the
origin.
-/
theorem circleAverage_log_norm_trailingCoeff_of_zero_meromorphicOrderAt (h : meromorphicOrderAt f 0 = 0) :
    circleAverage (fun a ↦ log ‖meromorphicTrailingCoeffAt (f · - a) 0‖) 0 1
...
/--
Circle average of the function `fun a ↦ log ‖meromorphicTrailingCoeffAt (f · -
a) 0‖` that appears in Cartan's formula, in case where `f` has a pole at the
origin.
-/
theorem circleAverage_log_norm_trailingCoeff_of_neg_meromorphicOrderAt (h : meromorphicOrderAt f 0 < 0) :
    circleAverage (fun a ↦ log ‖meromorphicTrailingCoeffAt (f · - a) 0‖) 0 1
      = log ‖meromorphicTrailingCoeffAt f 0‖ := by
