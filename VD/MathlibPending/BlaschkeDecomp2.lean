...
theorem divisor_congr_codiscreteWitin_closedBall_prod_canonicalFactor_smul {f g : ℂ → E}
  ...
  · have η₁ : AnalyticAt ℂ (∏ᶠ (u : ℂ), canonicalFactor R u ^ (-(divisor f (ball 0 R)) u)) x := by
      ...
      · apply AnalyticAt.zpow _ (canonicalFactor_ne_zero ha (by aesop) (by aesop))
...
theorem congr_codiscreteWitin_closedBall_prod_canonicalFactor_mul_prod_smul {f : ℂ → E}
  ...
  simp_rw [← divisor_congr_codiscreteWitin_closedBall_prod_canonicalFactor_smul (pos_of_mem_ball hw) h₁f h₁g h₂g h₃g]
  simp_all [← smul_assoc]
