...
/-
Establish positivity, parity and monotonocity of the Nevanlinna functions.
-/
...
-/
theorem proximity_nonneg {f : ℂ → ℂ} {a : WithTop ℂ} :
    0 ≤ proximity f a := by
  by_cases h : a = ⊤
  all_goals
    intro r
...
-/
theorem logCounting_nonneg {r : ℝ} {f : ℂ → ℂ} {a : WithTop ℂ} (hr : 1 ≤ r) :
    0 ≤ logCounting f a r := by
  by_cases h : a = ⊤
...
-/
theorem characteristic_nonneg {r : ℝ} {f : ℂ → ℂ} {a : WithTop ℂ} (hr : 1 ≤ r) :
